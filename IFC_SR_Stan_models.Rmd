---
title: "Interior Faser Coho Stock-Recruitment Models"
author: "Colin Bailey"
date: "2024-07-18"
output: html_document
---

The purpose of this Rmarkdown document is to provide a reproducible script for generating IFC stock-recruitment (S-R) parameters. Following the Holt et al. 2023 paper, there are two separate sets of IFC coho Ricker models used to estimate S-R parameters, after which these parameter estimates are combined by estimating means and quantiles from the combined posteriors of both model sets. The first model set uses a traditional Ricker model modified to account for a) 2 different ages at return, and b) a survival covariate using a survival index based on coded wire tag recoveries. From the first set of models (1 model for each CU), Srep (spawners at replacement, aka carrying capacity) was calculated for each CU and then multiplied by 1.4 (as per Holt et al. 2023) to create informative priors for Srep in the second set of models. The SD of these informative priors was set to 1414 as per Holt et al. 2023. Srep became an estimated parameter with an informative prior in the second set of models, and beta was calculated using the other estimated parameters in the model.

***NOTE - the code has been updated so that the model with the strong prior on capacity is still estimated, but the model no longer affects the reported parameter estimates, simulation, and ice cream plots. The influence of this model on the IFC FSAR results was removed after it was identified as a violation of the statistical assumption that prior information is not based on the data used to fit the model. Future work should revisit if there is good evidence that a prior on capacity is required, and b) if so, use independent data to develop the prior for capacity***

Loading the necessary packages and data to run the models in later code chunks.
...
Preparing the data for use in Stan models.
```{r data manipulation}
...
Simple linearized Ricker models with a survival covariate. The survival covariate is the estimated smolt-to-adult survival as measured from coded wire tag recoveries (see the parameter "gamma" and data "surv_3" and "surv_4" in the Stan model). There is a separate model for each of the 5 CUs as previous work by Holt et al. (2023) indicate that hierarchical models with random intercepts by CU performed more poorly.
```{r Ricker Models w/out Srep cap}
...
srdat$pred <- mf[36:140,1]
srdat$pred_rec <- mf[141:245,1]
...
Simple linearized Ricker models with a survival covariate and a strong prior on spawners at replacement (Srep), which limits the value of beta (see the transformed parameters section for the relationship between beta and srep). There is a separate model for each of the 5 CUs as previous work by Holt et al. (2023) indicate that hierarchical models with random slopes by CU performed more poorly. This model is kept as a reference, but does not influence the main results of the current 2022 IFC stock assessment.
```{r Ricker Models w/ Srep cap}
...
srdat$pred_cap <- mf[36:140,1]
srdat$pred_rec_cap <- mf[141:245,1]
...
Extracting parameter posteriors from the no-cap and prior cap models to solve for sgen, including mean and credible interval .
```{r calculate mean and CIs for sgen}
...
Extracting posteriors from the no-cap and prior cap models to calculate the average parameter values and credible intervals for 80% smsy and umsy. 
```{r calculate mean and CIs for 80% smsy & umsy}

#smsy summary - turned off the combination of the prior_Cap model posterior, so now the results are solely influenced by the baseline Ricker curve
smsy_nc <- as.data.frame(extract(fit_nocap, pars = "smsy_80")$smsy_80)
smsy_c <- as.data.frame(extract(fit_cap, pars = "smsy_80")$smsy_80)
smsy <- smsy_nc #rbind(smsy_nc, smsy_c)
...
#umsy summary - turned off the combination of the prior_Cap model posterior, so now the results are solely influenced by the baseline Ricker curve
umsy_nc <- as.data.frame(extract(fit_nocap, pars = "umsy")$umsy)
#umsy_c <- as.data.frame(extract(fit_cap, pars = "umsy")$umsy)
umsy <- umsy_nc #rbind(umsy_nc, umsy_c)
...
```{r calculate 1 gen mean escapement}
cd <- read.csv("Data_in/IFCoho_escpByCU.csv")
head(cd)
cd5yr <- as.data.frame(cd %>% group_by(ReturnYear) %>% summarize(sum(Escapement)) %>% filter(ReturnYear %in% c(2015:2022)))

out_means <- rep(0,5)

for(i in 4:8){
  x <- c(cd5yr[i-2, 2],cd5yr[i-1, 2],cd5yr[i, 2])
  out_means[i-3] = exp(mean(log(x)))
}
